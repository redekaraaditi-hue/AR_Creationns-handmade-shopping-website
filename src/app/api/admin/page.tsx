  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setIsEditing(false);
        const updatedProds = await (await fetch("/api/admin/products")).json();
    ...
    <div className="min-h-screen bg-stone-50 text-[#072428] font-sans pb-16">
      ...
      <header className="bg-[#072428] text-white px-6 py-4 flex justify-between items-center shadow-md">
        ...
        <div className="flex items-center space-x-4">
          ...
          <Link href="/" className="text-xs text-stone-300 hover:text-white transition-colors">
            View Live Store →
          </Link>
        </div>
      </header>
      ...
      {/* Navigation Tabs */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 flex space-x-3">
        <button
          onClick={() => setActiveTab("orders")}
          className={`px-5 py-2.5 text-xs font-bold rounded-xl transition-all shadow-sm ${
            activeTab === "orders"
              ? "bg-[#072428] text-amber-300 border border-amber-500/30"
